<template>
  ...
  <div v-if="isModalPostUpdateOpen" class="modal-container">
    <div class="modal-content" @click.stop>
      <div class="modal-inner">
        <!-- <PostDetail v-if="isModalPostDetailOpen" :id="selectedPostId"></PostDetail> -->
        <PostUpdate v-bind:updatePost="updatePost" @closePostUpdateModal="closePostUpdateModal"
        ...
        <div v-for="post in userPosts" :key="post.id" class="post-item">
          ...
          <div class="post-info">
            ...
            <div>
              ...
              <button style="z-index: 500;" class="delete-button" @click="openPostDeleteModal(post.id)">삭제</button>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    openPostDeleteModal(id) {
      if (confirm("정말로 게시글을 삭제하시겠습니까?") == true) {
        axiosInstance.delete('/post/' + id + '/delete').then(response => {
